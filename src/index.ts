...
export const connector = async () => {
    ...
    const provisionEntitlement = async (action: AttributeChangeOp, account: Account, entitlement: string) => {
        ...
        if (action === AttributeChangeOp.Add) {
            logger.info('add entitlement for the identity')
    ...
    return (
        createConnector()
            .stdTestConnection(async (context: Context, input: undefined, res: Response<StdTestConnectionOutput>) => {
                ...
                logger.info('access token is' + response1)
                if (response.status != 200 || typeof response1 !== 'string') {
                    throw new ConnectorError(
                        'Unable to connect to IdentityNow! Please check your Username and Password'
                    )
                } else {
            ...
            .stdAccountRead(
                async (context: Context, input: StdAccountReadInput, res: Response<StdAccountReadOutput>) => {
                    logger.info('Account Read' + input)
                    const workgroups: any[] = await getWorkgroups()
            ...
            .stdEntitlementList(async (context: Context, input: any, res: Response<StdEntitlementListOutput>) => {
                ...
                for (const w of response2.data) {
                    logger.info('inside for loop for response2')
